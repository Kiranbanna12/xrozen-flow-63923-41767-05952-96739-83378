...
export const UniversalVideoPlayer = forwardRef<any, UniversalVideoPlayerProps>(
  ({ url, onTimeUpdate }, ref) => {
    ...
    if (platform === 'youtube') {
        ...
        // Handle OneDrive URLs - convert to embed format
        ...
        if (embedUrl.includes('onedrive.live.com')) {
          ...
          embedUrl = embedUrl.replace('redir?', 'embed?');
          
          // Add embed parameter if not present
          if (!embedUrl.includes('em=')) {
            embedUrl += embedUrl.includes('?') ? '&em=2' : '?em=2';
          }
        }
        
        // Handle SharePoint URLs
        if (embedUrl.includes('sharepoint.com')) {
