...
export function useNotifications() {
  ...
  useEffect(() => {
    ...
    // Auto-refresh notifications every 30 seconds if authenticated
    const refreshInterval = setInterval(() => {
      if (apiClient.isAuthenticated()) {
        console.log('🔧 useNotifications: Auto-refreshing notifications');
        updateUnreadCount();
      }
    }, 30000);
    ...
    const handleNewNotification = (notification: Notification) => {
      console.log('🔧 useNotifications: New notification received:', notification.title);
    ...
    const handleNotificationUpdate = (notification: Notification) => {
      console.log('🔧 useNotifications: Notification updated:', notification.id);
    ...
    const handleNotificationDelete = (data: { id: string }) => {
      console.log('🔧 useNotifications: Notification deleted:', data.id);
