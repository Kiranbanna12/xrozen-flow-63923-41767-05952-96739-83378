  ...
  const loadPerformanceData = async () => {
    try {
      const [metricsData, healthData] = await Promise.all([
        ...
        apiClient.getPerformanceHealth()
      ]);

      if (metricsData) {
        setHealth(metricsData.systemHealth || healthData);
        setStatistics(metricsData.statistics || statistics);
        
        // Create mock metrics history for charts (since we don't have historical data yet)
        const now = new Date();
        ...
        const queryTimes = labels.map(() => Math.random() * 100 + 50);
        const errorRates = labels.map(() => Math.random() * 5);
                    ...
                    {statistics.slowQueries.map((query, idx) => (
                      <div key={idx} className="border rounded p-3">
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="font-medium">Query #{query.id}</p>
                          ...
                          <Badge variant="destructive">{query.execution_time}ms</Badge>
                        </div>
                      </div>
                    ))}
                  </div>
                    ...
                    {Object.entries(statistics.errorsByType).map(([error, count]) => (
                      <div key={error} className="border rounded p-3">
                        <div className="flex justify-between items-center">
                          ...
                          <Badge>{String(count)} occurrences</Badge>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <TabsContent value="operations">
          <Card>
            ...
            <CardContent>
              <ScrollArea className="h-[400px]">
                <div className="space-y-2">
                  {statistics.recentOperations?.slice(0, 50).map((operation, idx) => (
                    <div key={idx} className="border rounded p-3">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-medium">{operation.operation_type} → {operation.details}</p>
                          <p className="text-sm text-muted-foreground">
                  ...
                  {(!statistics.recentOperations || statistics.recentOperations.length === 0) && (
                    <p className="text-center text-muted-foreground py-8">
