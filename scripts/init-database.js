...
async function initializeDatabase() {
  ...
  try {
    ...
    // Editors table with data isolation
    db.exec(`
    ...
    // Clients table with data isolation
    db.exec(`
    ...
    // Project last read tracking for unread counts
    db.exec(`
    ...
    // Notifications table (Enhanced for role assignments)
    db.exec(`
    ...
    // User relationships table for reciprocal relationships
    db.exec(`
    ...
    // Pending invitations table for email-based invitations
    db.exec(`
    ...
    // Project access table for proper project permissions
    db.exec(`
    ...
    db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_project_last_read_project_id ON project_last_read(project_id);
      CREATE INDEX IF NOT EXISTS idx_project_last_read_user_id ON project_last_read(user_id);
      CREATE INDEX IF NOT EXISTS idx_project_last_read_updated_at ON project_last_read(updated_at);
    ...
    // Create sample AI models (skip if table schema doesn't match)
    try {
      const aiModels = [
        {
          id: randomUUID(),
      ...
      for (const model of aiModels) {
        db.prepare(`
          INSERT OR IGNORE INTO ai_models (id, model_id, model_name, provider, is_active, priority, created_at, updated_at)
          VALUES (?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
        `).run(model.id, model.model_id, model.model_name, model.provider, model.is_active, model.priority);
      }

      console.log('✅ Sample AI models created');
    } catch (error) {
      console.log('⚠️  AI models table schema mismatch, skipping sample data');
    }
    ...
    // Try to get AI models count if table exists with correct schema
    try {
      stats.aiModels = db.prepare('SELECT COUNT(*) as count FROM ai_models').get().count;
    } catch (error) {
      stats.aiModels = 'N/A (schema mismatch)';
    }
    ...
    console.log(`  AI Models: ${stats.aiModels}`);
